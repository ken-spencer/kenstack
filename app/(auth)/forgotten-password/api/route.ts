...
export async function POST(request: Request) {
  ...
  if (request.headers.get('content-type') !== "application/json") {
      const response = NextResponse.json({
        type: "error",
        message: "Invalid request. Please contact support if this problem persists.",
      })
  ...
  const errorResponse = NextResponse.json({
    type: "error",
    message: "We experienced a problem resetting your password. Please try again later.",
  })
  ...
  if (results.length >= 3) {
    auditLog(request, `Forgotten Password; Too many requests`, { user });
      ...
      const response = NextResponse.json({
        type: "error",
        message: "We have recieved to many requests to reset this user's password. Please try again later.",
      })
