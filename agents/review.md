...
Git index state is operator housekeeping, not a code-review concern. Never mention whether files are staged, unstaged, partially staged, or untracked in review findings, summaries, or commit-readiness recommendations. Review the combined intended diff. Discuss staging only when the user explicitly asks to stage, unstage, or audit the index.

Review unit tests against **When not to create a unit test** in the repository `AGENTS.md`. Remove tests
that preserve temporary scaffolding, interface copy, implementation structure, duplicated enforcement,
or otherwise incidental behavior instead of a durable contract. Do not count a test as valuable merely
because it passes or increases coverage; preserve tests that protect meaningful observable behavior at
the appropriate boundary.

Match the explanation to the actual size of the issue. Start with the simplest plain-language meaning and add detail only when it changes the decision or fix. Do not use severity language, architectural framing, or multi-step explanations for housekeeping, obvious fixes, or speculative risks. If an explanation could make a small issue sound like a larger problem, shorten it before reporting.
...
For every new or changed `agents/migrations.md` entry, verify that the old behavior
shipped as a Kenstack contract, the implementation change is independently justified
for shared consumers, and downstream action is genuinely required. Host-local or
entirely uncommitted work must update its callers directly without creating cross-site
migration guidance.
...
- List every added or changed function, helper, wrapper, factory, and adapter in each changed TypeScript file, then inspect its full body and every call site. Inline it when it only forwards arguments to another function, returns a property or library call, renames a result, changes only a TypeScript signature, or discovers a value already fixed by the surrounding contract. A test that only exercises the helper itself is not a production consumer and does not justify the layer. Keep the function only when it adds current policy, validation, normalization, transformation, meaningful reuse, stable callback identity, or a deliberate runtime/public boundary.
- Check every newly added outermost function against the leading-comment
  requirement in `../agents.md`. Treat missing, redundant, overlong, or
  incorrectly wrapped comments as review findings.
- Check optional-feature code against the readability, separation-of-concerns,
  and explicit-branch requirements in `../agents.md`.

## Type Shape

- Apply the complete **Review Pass** in `agents/typescript.md` to every touched TypeScript file, regardless of the review's requested emphasis. Do not restate or substitute a shorter type checklist here.
- Type review is incomplete until the removal experiment has been performed. In the review result, identify every remaining new or changed cast, assertion, and type export with its concrete boundary, or state that none remain.
...
- Flag type-checking rats nests: repeated `typeof`, `in`, `Array.isArray`, or property checks that narrow several fields from one external value. Prefer one schema parse at that boundary when the expected shape is known. Keep a direct check when only one or two values need narrowing.
...
- Flag overqualified variable names when the qualifier does not distinguish another live value or a meaningful state. Prefer the shortest unambiguous name; prefixes such as `normalized`, `resolved`, `current`, or `verification` must add enough meaning to justify their repeated reading cost.
- Check names that describe a current consumer or location rather than the operation
  or value, especially qualifiers such as `ForInput`, `ForAdmin`, `ForDisplay`, and
  `ForPage`. Keep the qualifier only when it identifies a distinct contract or
  representation. If the same operation could move to another caller without changing,
  name the operation itself.
- Treat variable and function names made from more than two words as a strong signal of needless verbosity. Strongly prefer one word when its meaning is clear in context, use two only when they materially disambiguate, and keep longer names only when every word prevents genuine ambiguity. Do not repeat context already supplied by the file, owner, type, or surrounding function.
- When two live values share a name, preserve the caller-facing or public API name. Alias or qualify the secondary internal, parsed, or external value instead. Do not make familiar API vocabulary indirect merely to accommodate a local name collision.
...
- For new or changed UI, compare it with the closest established screen or repeated control before accepting its styling. Match the existing spacing, density, responsive behavior, labels, dividers, borders, and action placement unless the interaction has a clear reason to differ. Treat unexplained visual drift between neighboring patterns as a review finding.
...
- Check every optional argument at changed call sites. Keep it only when it changes behavior or adds information that is not already present. For `deps.error(...)`, put diagnostic details in the message and use `context` only for structured filtering or correlation; never repeat the message there.
...
- Apply the state-ownership rule in `agents/ui.md` when reviewing form or query state.
...
- Review every new or changed render/load-path `throw` that can be reached by stored data. Check whether staff could repair the condition if the admin interface remained available. Do not turn a repairable record problem into a route-level exception; render an accurate warning and keep the controls needed to repair it accessible. Keep the throw when the condition is an unrecoverable programmer/configuration invariant or continuing would cross a security boundary.
- Check user-facing error messages against what the failing branch actually proves. Flag specific explanations that guess or misstate the cause. Use a generic accurate message when the code only knows that the operation failed; name a cause only when that cause has been established.
- Distinguish useful logging from operator alerting. Expected suspicious activity, such as low risk scores, wrong actions, invalid user input, or ordinary provider rejections, may be worth sanitized permanent logging but must not call `deps.error(...)`. Reserve `deps.error(...)` for failures that require operator action; user-caused failures must never generate operator email.
- Check errors sent to `deps.error(...)` and permanent server logs for useful standalone messages. The error text must identify the failure and any known status, code, or cause needed to understand it without digging through structured context. Keep context for filtering and supporting detail, not as the only explanation.
...
- Treat catalogue descriptions, content fields, labels, and similar admin-authored values as user-facing unless the product explicitly proves otherwise. Flag AI commentary, provenance, import notes, assumptions, review instructions, and implementation language stored in those fields; keep that context in code comments, import metadata, or planning documentation instead.
