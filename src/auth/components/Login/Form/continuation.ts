...
// verification, which re-enters the flow at the login step's own URL.
// Standalone login leaves for the server-selected destination.
...
export function resolveReturnTo({ anchor, entryPath, mode }: Continuation) {
  if (mode === "embedded") {
    ...
    return entryPath + (params.size ? `?${params}` : "") + `#${anchor}`;
  }
