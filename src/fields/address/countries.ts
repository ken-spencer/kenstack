...
type AddressLabels = Pick<
  SupportedCountry,
  "localityLabel" | "postalCodeLabel" | "regionLabel"
>;
...
const defaultAddressLabels = {
  ...
  regionLabel: "Region",
} satisfies AddressLabels;

const addressLabelsByCountry: Record<string, AddressLabels> = {
  AU: {
...
export function getAddressLabels(countryCode: unknown) {
  return (
    addressLabelsByCountry[normalizeCode(countryCode)] ?? defaultAddressLabels
  );
}
...
export function formatSupportedRegion(
  ...
  const regionName =
    findSupportedRegion(country, regionValue)?.name ?? regionValue;
  const countryName = country?.name ?? countryValue;
...
export function formatSupportedRegionCode(
  ...
  return countryValue && regionValue
