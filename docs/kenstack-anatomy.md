...
`docs/module-anatomy.md` covers the inside of any registered module, including host modules;
cross-cutting ownership, helper, and file-shape rules live in `docs/code-organization.md`.
...
Each top-level `src` area has one owner. Add a new area only when no existing owner can contain an
...
- `logger/` — audit logging and its request metadata implementation. Call its functions directly;
  logging configuration belongs inside this owner.
...
There is no top-level `zod/` owner. Validation schemas belong to their field or other domain owner,
which may export them for external consumers; the implementation library does not create ownership.

Top-level files and folders outside these ownership descriptions are transitional, not new
destinations. Resolve them toward a named owner.
...
`defineClient(...)` receives only the bare isomorphic definitions and neither imports nor resolves
editor components. Relation forms pass the field subset they render and their `prefix` directly to
`defineFormFields(...)`; there is no separate resolved component-bearing field map. Several fields may
share a primitive directly, without wrappers for symmetry.
...
`@kenstack/forms/*` component entry points and controls, which may forward to field-owned
implementations and remain intentional public APIs.

Use `field({ kind, zod, default })` for a concrete field definition. A one-off module-local field
registered by property may omit `kind`; `defineFields(...)` normalizes its property name into the
resolved kind. Its `default` retains the literal initial value; `zod` independently owns the field's
accepted input and output types. Every reusable field helper is created with `defineField(...)` and
declares an explicit semantic kind; when `defineField(...)` cannot express a reusable helper, extend it.
...
already present. Derive editable and saved value types with `z.input` and `z.output` from the field's
schema.
...
a semantic kind, register the corresponding implementations; isomorphic loaded-value flags are not a
substitute. The page editor owns its supported inline field set; generic fields declare no page-editor
capabilities.
...
result of `field(...)` or of calling a field factory such as `textField(...)`, never the factory
function itself. Server field-kind registries accept only the resolver returned by one of these helpers
at the typed boundary; a raw `ServerField` object is not a typed registration. This keeps kind, default,
input, and value contracts derived from the isomorphic owner. Spread configurable additions before
`kind` so the owner's canonical identity cannot be replaced.
